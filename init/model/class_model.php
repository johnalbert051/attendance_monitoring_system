...
<?php
	...
	class class_model{
		...
		public function add_employee($employee_idno, $password, $first_name, $middle_name, $last_name,  $gender,$school_year, $department, $codeContents){
	       $stmt = $this->conn->prepare("INSERT INTO `tbl_employee` (`employee_idno`, `password`, `first_name`, `middle_name`, `last_name`,  `gender`, `school_year`, `department`, `qr_code`) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)") or die($this->conn->error);
			$stmt->bind_param("sssssssss", $employee_idno, $password, $first_name, $middle_name, $last_name,  $gender,$school_year, $department, $codeContents);
			if($stmt->execute()){
				$stmt->close();
				$this->conn->close();
				return true;
	    ...
	    public function fetchAll_employees(){ 
		  ...
		  public function fetchAll_admin_account(){ 
		  ...
		  public function fetchAll_Schoolyear(){ 
        ...
        public function edit_employee($employee_idno,  $first_name, $middle_name, $last_name, $bdate, $caddress, $cnumber,  $gender, $civilstatus, $datehire, $school_year, $department, $employee_id){

			$sql = "UPDATE `tbl_employee` SET  `employee_idno` = ?, `first_name` = ?, `middle_name` = ?,  `last_name` = ? ,  `bdate` = ?,  `complete_address` = ?,  `cnumber` = ?,  `gender` = ?,  `civilstatus` = ?,  `datehire` = ?, `school_year` = ?,    `department` = ? WHERE employee_id = ?";
			 $stmt = $this->conn->prepare($sql);
			$stmt->bind_param("isssssssssssi", $employee_idno,  $first_name, $middle_name, $last_name, $bdate, $caddress, $cnumber,  $gender, $civilstatus, $datehire, $school_year, $department, $employee_id);
			if($stmt->execute()){
				$stmt->close();
				$this->conn->close();
				return true;
		...
		public function add_users($fname, $username,$password_encrypted){
			$stmt = $this->conn->prepare("INSERT INTO `tbl_admin` (`full_name`, `username`,`password`) VALUES(?, ?,?)") or die($this->conn->error);
			 $stmt->bind_param("sss", $fname, $username,$password_encrypted);
			 if($stmt->execute()){
				 $stmt->close();
				 $this->conn->close();
				 return true;
		...
		public function add_schoolyear($school_year){
			$stmt = $this->conn->prepare("INSERT INTO `school_year` (`school_year`) VALUES(?)") or die($this->conn->error);
		 ...
		 public function delete_schoolyear($id){
			...
			$sql = "DELETE FROM school_year WHERE id = ?";
			 $stmt = $this->conn->prepare($sql);
